...
脚本把「**配色**」与「**结构格式**」拆成两个独立维度，二者相乘共有 **9 × 9 = 81 种组合**。
...
- 可选参数：
  ...
  - `--theme <配色>`：默认配色（`blue`/`classic`/`tech`/`green`/`warm`/`dark`/`news`/`note`/`minimal`），
    预览页内仍可切换配色与格式任意组合
  - `--format <格式>`：默认结构格式（`sidebar`/`seal`/`chip`/`marker`/`card`/`night`/`news`/`note`/`minimal`）
...
- **配色与格式解耦**：文件顶部 `COLORS` 字典只放颜色/字体（无结构），`VARIANT_ORDER`/`VARIANT_LABELS`
  ...
  叠加结构样式（标题/引用/高亮）。`STYLES` 是 `{(color, variant): 样式字典}` 的笛卡尔积（9×9=81）。
  新增配色/格式只需分别改 `COLORS` 或 `_variant_overrides` 分支，并补 `COLOR_ORDER`/`VARIANT_ORDER` 列表。
- **预览页实时组合**：为支持 81 种组合又不预渲染 81 份正文，预览页把「解析后的块 JSON」与「全部组合
