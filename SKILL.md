...
- 表格 `| a | b |` + 分隔行（带边框与表头底色）
- 分隔线 `---` （渲染为居中圆点装饰线）
- 分栏分隔线 `SECTION 01`（独立成行：大写 `SECTION` + 空格 + 数字）：渲染为居中 "— SECTION 01 —" 装饰分栏，适合商业/报告风的分节
...
- 对比双栏 `> [!compare] A标题 | B标题`：引用块首行以 `[!compare]` 开头，紧接写两栏标题用 `|` 分隔；其下两栏内容之间用单独一行 `> |||` 分隔。渲染为并排两栏对照卡（左栏主色标题、右栏绿色标题，制造对照），适合「传统 vs 优秀」「旧方案 vs 新方案」类对照。
