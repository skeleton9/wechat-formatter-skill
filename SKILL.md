...
3. **块级容器一律用 `<section>`，绝不用 `<div>`** — 公众号编辑器（ProseMirror）有**标签白名单**，
   只保留 `section / p / span / table(+tr,td,th) / img / br / strong / em` 与内联 `style`。
   **`<div>` 会被直接剔除**，连同它身上的内联 style（边框/底色/圆角）一起被吃掉——表现为
   "div 没了、都是 p"。这是第三方排版工具（秀米/135/壹伴）把 div 全转 section 的根本原因。
4. **CSS 简写会被净化，一律写长写** — 公众号粘贴时 `border:1px solid #xxx`、`background:#xxx`
   这类**四边简写经常整体丢失**（边框/底色消失，但浏览器里正常）。必须拆成长写：
   `border-top/right/bottom/left:1px solid #xxx`、`background-color:#xxx`。本工具已默认写长写
   （脚本里所有 `border:` 简写均已展开为四边），新增样式时务必沿用。
5. **`<h1>~<h6>` 会被强制还原块级** — 无视 `display:inline`，会把"数字徽章+标题""色条+标题"
   这类期望同行的结构拆成两行。凡需徽章/色条与标题严格同行，用 `<span>` 承载文字、用 `<section>`
   承载结构，不要依赖 h 标签的 inline-block。本工具的编号大标题 / 编号贴士卡已改用 `table` 双列
   （数字在 td 内 span）规避此坑。
6. **引用框自动分类** — 根据内容关键词自动判断颜色：
...
- **编号贴士卡** `> [!01] 标题` + 后续行作描述：引用块首行以 `[!数字]` 开头，渲染为**带浅底 + 左主题色条的小卡片**——大字号数字（约 38px）+ 加粗标题 + 描述文字，都进卡片内（带边框）。适合「一句话提醒 / 要点归纳」等**短内容**；数字写两位（01、02…）更整齐。
- **编号大标题** `## 01 章节标题`（注意是 H2 标题，不是 `>` 引用）：H2 以「数字 + 分隔符（空格/./、/｜）」开头时，渲染为「大字号数字（约 44px 主题色）+ 章节标题 + 1px 分隔线」的独立头部，**其下是普通满宽正文段落**（与普通正文同款样式，不进卡片、不使用 `>` 引用，满宽阅读）。适合「01 / 02 / 03」式逐条展开的分论点、步骤、清单等**大段正文**。**大段正文请务必用这种 H2 写法，不要用带边框的编号卡**。
...
- **WorkBuddy 预览面板是 sandbox iframe**：`navigator.clipboard` 不可用、`execCommand('copy')`
  常被沙箱拦截。在预览面板里点「复制正文」按钮，粘贴到公众号往往只剩纯文本（或根本没写进
  系统剪贴板）。**此路不可靠。** 可靠路径是：用**系统浏览器**双击生成的 `.wechat.html`
  （或 `--serve` 起本地服务后打开 http://localhost:8000），在真实浏览器里点「复制正文」→
  公众号后台 `Ctrl/⌘+V`，格式 100% 保留。
- **多列布局用 `<table>`**：本工具的对比双栏（`> [!compare]`）、编号大标题、编号贴士卡、普通
  表格均用 `<table>`（公众号对 table 支持最稳）。**不要用 flex / grid / `inline-block` 多列**，
  公众号会塌成竖排并丢样式。
- **列表箭头用真实文字 `→ `**：若以后新增带箭头的清单样式，箭头务必是真实字符（伪元素
  `::before` 不进剪贴板，粘过去箭头消失）。普通 `ul/li` 列表若需要边框/分隔线，li 的边框会被剥，
  应改 `<section>`+`border-bottom`。

### 生成后校验清单（每次改完样式都过一遍）
- [ ] 生成的 HTML 里**无 `<div>` 进入正文区**（块级容器全是 `<section>`；UI 外壳的 div 不进剪贴板，可忽略）
- [ ] 无 `border:` / `background:` **四边简写**残留（应都是 `border-top/right/bottom/left` 与 `background-color`）
- [ ] 所有块级样式写在 `<section>` 的 `style` 上，而非外部 `<style>`/class
- [ ] 多列结构是 `<table>`，`td` 上带 `border`/`background-color`/`text-align`
- [ ] 标题/徽章用 `<span>` 或 `table` 双列承载，无依赖 h 标签 `inline-block` 的同行结构
- [ ] 在**系统浏览器**（非预览面板）实测一次复制 → 公众号粘贴，确认边框/底色/圆角/箭头都在
